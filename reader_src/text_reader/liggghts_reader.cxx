...
#define MSG(x) vtkWarningMacro(x)
	...
	if (IDpos>=0) {
		...
		scalars[n_scal]->SetName("Point ID");
		scalars[n_scal]->SetComponentName(0,"Point ID");
		s_IDpos=n_scal;
		n_scal++;
	}
...
int liggghts_reader::RequestInformation(vtkInformation *request, vtkInformationVector **inputVector, vtkInformationVector *outputVector)
{
	vtkInformation *outInfo = outputVector->GetInformationObject(0);
